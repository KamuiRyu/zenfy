...
func GetValidationMessage(c *fiber.Ctx, tag, field, param, errMsg string) string {
	...
	switch tag {
	case "required":
		msg = messages.GetMessage(messages.RequiredField)
	case "min":
		msg = messages.GetMessage(messages.MinLength, param)
	case "max":
		msg = messages.GetMessage(messages.MaxLength, param)
	case "email":
		msg = messages.GetMessage(messages.EmailFormat)
	case "url":
		msg = messages.GetMessage(messages.URLFormat)
	case "uuid":
		msg = messages.GetMessage(messages.UUIDFormat)
	case "numeric":
		msg = messages.GetMessage(messages.NumericFormat)
	case "boolean":
		msg = messages.GetMessage(messages.BooleanFormat)
	default:
		msg = errMsg
	}
