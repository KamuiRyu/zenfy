...
func (r *userRepoImpl) GetByID(id int) (*model.User, error) {
	...
	u := &model.User{}
	row := r.db.QueryRowContext(ctx, "SELECT id, name, email, password, created_at, verified FROM users WHERE id = ?", id)
	if err := row.Scan(&u.ID, &u.Name, &u.Email, &u.Password, &u.CreatedAt, &u.Verified); err != nil {
...
func (r *userRepoImpl) Create(u *model.User) error {
	ctx := context.Background()
	row := r.db.QueryRowContext(ctx, "INSERT INTO users(name,email,password) VALUES(?, ?, ?) RETURNING id", u.Name, u.Email, u.Password)
...
func (r *userRepoImpl) GetByEmail(email string) (*model.User, error) {
	ctx := context.Background()
	u := &model.User{}
	row := r.db.QueryRowContext(ctx, "SELECT id, name, email, password, created_at, verified FROM users WHERE email = ?", email)
	if err := row.Scan(&u.ID, &u.Name, &u.Email, &u.Password, &u.CreatedAt, &u.Verified); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
